...
export const createTimer = async (req, res) => {
  try {
    ...
    if (!title || !duration || !date) {
      return res
        .status(400)
        .json({ message: "Title, duration, and date are required" });
    }
...
// to delete all timer data
export const deleteAllTimers = async (req, res) => {
  try {
    const userID = req.user._id;

    // to find timers according to specific user
    const timers = await Timer.find({ user: userID });

    if (!timers || timers.length === 0) {
      // also ensure tasks are not an empty array
      return res.json({
        error: "No timer information found",
      });
    }

    // delete all timer information for the user
