...
export const UserContextProvider = ({ children }) => {
  ...
  const registerUser = async (e) => {
    ...
    if (
      ...
      toast.error(
        "Please entrer a valid email and password (minimum 8 characters"
      );
      return;
    }
  ...
  const logoutUser = async () => {
    try {
      ...
      localStorage.removeItem("token"); // If using localStorage for auth
  ...
  const sendCode = async (email) => {
    try {
      const response = await axios.post(
        `${serverUrl}/api/v1/send-verification-code`,
        { email }
      );
  ...
  const verifyCode = async (email, verificationCode) => {
    try {
      const response = await axios.post(`${serverUrl}/api/v1/verify-code`, {
        email,
        verificationCode,
      });
      // console.log(response.data.message);\
  ...
  const requestResetCode = async (email) => {
    try {
      const response = await axios.post(
        `${serverUrl}/api/v1/request-password-reset`,
        { email }
      );
      console.log(response.data.message);
  ...
  const verifyResetCode = async (email, resetCode) => {
    try {
      const response = await axios.post(
        `${serverUrl}/api/v1/verify-password-reset-code`,
        {
          email,
          resetCode,
        }
