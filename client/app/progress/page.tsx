...
const TimeBox: React.FC<TimeBoxProps> = ({ label, time, className }) => (
  <div
    className={`flex-1 bg-gray-300 p-4 rounded-lg flex flex-col items-center justify-center ${className}`}
  >
...
>>>>>>> parent of 1e41dca (fixed progress page studytime not showing)
  ...
  const calculateTotalTime = (
    ...
    timers.forEach((timer) => {
      ...
      if (
        period === "daily" &&
        timerDate.toDateString() === now.toDateString()
      ) {
        ...
        if (
          timerDate.getMonth() === now.getMonth() &&
          timerDate.getFullYear() === now.getFullYear()
        ) {
          totalSeconds += totalDurationInSeconds;
        }
  ...
  useEffect(() => {
    ...
    const weeklyTasks = tasks.filter(
      (task: { duedate: string | Date; completed: string }) => {
      ...
      <div className="flex space-x-6">
        ...
        <div className="flex-1 bg-gray-800 p-6 rounded-xl shadow-lg flex flex-col items-center">
          ...
          <div className="flex space-x-6 text-white  ">
            <TimeBox
              label="Daily"
              time={calculateTotalTime("daily")}
              className="w-[100px] bg-gray-600 rounded-lg p-4 shadow-md text-center bg-white bg-opacity-20"
            />
            <TimeBox
              label="Weekly"
              time={calculateTotalTime("weekly")}
              className=" bg-gray-300  shadow-md text-center bg-white bg-opacity-20"
            />
            <TimeBox
              label="Monthly"
              time={calculateTotalTime("monthly")}
              className=" bg-gray-300  shadow-md text-center bg-white bg-opacity-20"
            />
          </div>
        </div>
        ...
        <div className="flex-1 bg-gray-800 p-6 rounded-xl shadow-xl flex flex-col items-center justify-center text-white">
          ...
          <div className="bg-white bg-opacity-20 p-2 rounded-lg shadow-md mb-4 w-full">
            <h1 className="text-xl font-extrabold text-center text-white ">
              Tasks:
              <br /> <span className="text-green-300">{totalTasks2}</span>
            </h1>
          </div>
        ...
        <div className="flex-1 bg-gray-800 p-6 rounded-lg shadow-lg flex flex-col items-center ">
          ...
          <div className="bg-white bg-opacity-20 text-base font-bold text-white p-7 py-2 rounded-md mb-7 shadow-md">
            Weekly Tasks Goal:{" "}
            <span className="font-bold">{weeklyTaskGoal}</span>
          </div>
          {/* Completion Title */}
          ...
          <table className="w-full bg-white bg-opacity-20 table-auto border-separate border-spacing-2 shadow-lg rounded-lg ">
            <thead>
              <tr className="bg-gray-700 bg-opacity-80">
                <th className="p-3 text-center font-medium-bold text-gray-100 rounded-lg">
                  Priority Level
                </th>
                <th className="p-3 text-center font-medium-bold text-gray-100 rounded-lg">
                  Event
