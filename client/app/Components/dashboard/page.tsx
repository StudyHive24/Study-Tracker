...
const getStudyInstructorMessages = (upcomingTasks: any[]) => {
  ...
  if (!upcomingTasks.length) {
    messages.push(
      "No tasks found. Use this time to plan ahead or add new tasks!.. "
    );
    return messages;
  }
  ...
  if (
    upcomingTasks[0] &&
    new Date(upcomingTasks[0].duedate).toDateString() === today.toDateString()
  ) {
    messages.push(
      `Your task "${upcomingTasks[0].title}" is due today! Make sure to complete it... `
    );
  }

  // Condition 2: More than 3 tasks due within 3 days
  const tasksDueInThreeDays = upcomingTasks.filter(
  ...
  if (tasksDueInThreeDays.length > 3) {
    messages.push(
      `You have ${tasksDueInThreeDays.length} tasks due within the next 3 days. Prioritize your work!.. `
    );
  }

  // Condition 3: Reminder to use Timer
  messages.push(
    "Don't forget to use the Timer to stay focused while studying!.. "
  );

  // Condition 4: High-priority task due within 3 days
  const highPriorityTasks = tasksDueInThreeDays.filter(
    (task: { priority: string }) => task.priority === "High"
  );
  if (highPriorityTasks.length > 0) {
    messages.push(
      `Your high-priority task "${highPriorityTasks[0].title}" is due within 3 days! Focus on it first... `
    );
  }
...
const Dashboard = () => {
  ...
  const completedTasks = tasks.filter(
    (task: { completed: string }) => task.completed === "yes"
  ).length;
  ...
  const todayTasks = tasks.filter(
    (task: any) =>
      new Date(task.endTime).toDateString() === today.toDateString()
  );
  const notcompletedtodayTasks = tasks.filter(
    (task: any) =>
      task.completed == "no" &&
      new Date(task.endTime).toDateString() === today.toDateString()
  );
  const nextTask = tasks
    .filter(
      (task: any) => task.completed == "no" && new Date(task.endTime) > today
    )
    .sort(
      (a: any, b: any) =>
  ...
  const completedPercentage =
    totalTodayTasks > 0
      ? Math.floor((completedTodayTasks.length / totalTodayTasks) * 100)
      : 0;
  const remainingTasks = totalTodayTasks - completedTodayTasks.length;
  const timerUsedToday = timerHistory.some(
    (entry: any) => new Date(entry.date).toDateString() === today.toDateString()
  );
    ...
    <div className="dashboard-container">
            ...
            <ul>
              {(() => {
                ...
                if (nextTask) {
                  ...
                  const formattedTime = nextTaskDate.toLocaleTimeString(
                    "en-US",
                    {
                      hour: "2-digit",
                      minute: "2-digit",
                  ...
                  const daysDifference = Math.ceil(
                    (nextTaskDate.getTime() - today.getTime()) /
                      (1000 * 60 * 60 * 24)
                  );
                  let dateMessage;
                  ...
                  notifications.push(
                    `Next task: "${
                      nextTask.title
                    }" is due ${dateMessage} at ${nextTaskTime.toLocaleTimeString(
                      [],
                      {
                        hour: "2-digit",
                        minute: "2-digit",
                ...
                if (completedPercentage === 0) {
                  ...
                  notifications.push(
                    "Great job! You've completed 100% of today's tasks."
                  );
                }
                ...
                if (!timerUsedToday) {
                  notifications.push(
                    "You haven't used the timer today. Use it to boost your productivity!"
                  );
                } else {
                  notifications.push(
                    "Great! You've used the timer today. Consider using it again to stay on track."
                  );
                }
      ...
      <div className="dashboard-content">
        ...
        <div className="grid-item chatbot">
          <img src="favicon.jpg" alt="Chatbot" className="chatbot-icon" />
        ...
        <div className="grid-item timer-history">
          {" "}
          {/* Added Timer History Section */}
          <h2>📌Timer History</h2>
          <table className="styled-tasks-table">
            <thead>
              <tr>
            ...
            <tbody>
              {timerHistory.length > 0 ? (
                timerHistory.map(
                  (
                    timer: { title: string; duration: number; date: string },
                    index: number
                  ) => (
                    <tr key={index}>
                      <td>{timer.title}</td>
          ...
          <Calendar
            ...
            tileContent={({ date, view }) => {
              ...
              if (view === "month") {
                ...
                return taskOnDate ? (
                  <div className="task-highlight">📌</div>
